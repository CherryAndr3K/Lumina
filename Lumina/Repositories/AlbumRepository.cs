...
namespace Lumina.Repositories
{
    ...
    {
        public IEnumerable<Albumes> GetAll()
        {
            var list = new List<Albumes>();
            ...
            while (rd.Read())
            {
                list.Add(new Albumes
                {
                    AlbumesId = rd.GetInt32(0),
        ...
        public bool Update(Albumes album)
        {
            using var cn = GetConnection();
            using var cmd = new SqlCommand(
            ...
            cmd.Parameters.AddWithValue("@a", (object?)album.Artista ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@g", (object?)album.Genero ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@id", album.AlbumesId);
            cn.Open();
